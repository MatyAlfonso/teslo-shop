...
const updateProduct = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    if (images.length < 2) {
        return res.status(400).json({ message: 'Is necessary to upload at least 2 images.' });
    }
    ...
    try {
        ...
        //TODO: Delete images on Cloudinary
...
const createProduct = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    try {
        ...
        if (productInDB) {
            return res.status(400).json({ message: 'Product already exists with the same slug.' });
        }
